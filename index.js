...
client.on("ready", async () => {
	...
	await console.log(chalk.whiteBright(`                                        Version - 1.0.0`));
...
client.distube.on("playSong", (message, queue, song) => {
  ...
  client.distube.on("addSong", (message, queue, song) => {
	let check = client.emojis.cache.find(emoji => emoji.name === "green_check");
  
	const embed = new discord.MessageEmbed()
	  .setColor("GREEN")
	  .setTitle(":notes: Song Added")
	  .setDescription(`${check} **Song:** \`${song.name}\` **-** \`${song.formattedDuration}\`\n**Added to the queue by** ${song.user}`)
	return message.channel.send(embed);
  });
  client.distube.on("playList", (message, queue, playlist, song) => {
	...
	const embed = new discord.MessageEmbed()
	  ...
	  .setDescription(`${check} **Playlist:** \`${playlist.title} (${playlist.total_items} songs)\`\n**Requested by:** ${song.user}\n**Now playing:** \`${song.name}\` **-** \`${song.formattedDuration}\``)
	return message.channel.send(embed);
  });
  ...
  client.distube.on("error", (message, err) => {
	let cross = client.emojis.cache.find(emoji => emoji.name === "red_cross");
  
	return message.channel.send(`${cross} **An error encountered:** ${err}`);
  });
...
async function startBot(time) {
	...
	setTimeout(() => {
		...
		bot.on("error", async() => {
			console.log(chalk.whiteBright(`${chalk.gray(`[${chalk.blueBright("*")}]`)} Minecraft bot: Couldn't relog now.`)); return
			tries++;
		})
